		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			...
			<div class="mt ofc">
				...
				<c:forEach var="product" items="${productList }">
					<li>
						<a href="productDetail.jsp" title="" target="_blank" class="pic"><img src="${product.webSite }" alt="${product.nameExHtml }" /></a>
						<dl>
							<!-- dt 10个文字+... -->
							<dt class="playname2"><a  href="productDetail.jsp" title="" target="_blank">${product.name}</a></dt>
							<!-- dt 25个文字+... -->
			...
			<div class="page pb15">
				<span class="r inb_a page_b">
					...
					<a href="/product/display/list.do?brandId=${productQuery.brandId }&amp;isShow=${productQuery.isShow}&amp;name=${productQuery.name}&amp;pageNo=1"><font size="2">首页</font></a>
					<a href="/product/display/list.do?brandId=${productQuery.brandId }&amp;isShow=${productQuery.isShow}&amp;name=${productQuery.name}&amp;pageNo=${productQuery.pageNo -1}"><font size="2">上一页</font></a>
					</c:if>
					<c:forEach var="currentPage" begin="1" end="${productQuery.totalPage}" >
						...
						<a href="/product/display/list.do?brandId=${productQuery.brandId }&amp;isShow=${productQuery.isShow}&amp;name=${productQuery.name}&amp;pageNo=${currentPage}">${currentPage}</a>
						</c:if>
					</c:forEach>
					<c:if test="${productQuery.pageNo ne productQuery.totalPage}">
					<a href="/product/display/list.do?brandId=${productQuery.brandId }&amp;isShow=${productQuery.isShow}&amp;name=${productQuery.name}&amp;pageNo=${productQuery.pageNo +1}"><font size="2">下一页</font></a>
					<a href="/product/display/list.do?brandId=${productQuery.brandId }&amp;isShow=${productQuery.isShow}&amp;name=${productQuery.name}&amp;pageNo=${productQuery.totalPage}"><font size="2">尾页</font></a>
					</c:if>
					共<var id="pageTotal" class="orange">${productQuery.totalPage}</var>页 到第<input type="text" id="number" name="number" class="txts" size="3" />页 <input type="button" id="skip" class="hand btn60x20" value='确定' />
				</span>
			</div>
	</div>
</div>
